...
# Configure the page
st.set_page_config(page_title="Fasilitas Kesehatan Indonesia 📌", layout="wide")

# Set title and custom styles
# st.title(":bar_chart: ANALYSIS DATA STATUS KESEHATAN DI INDONESIA 👩‍⚕️")
st.markdown(
    '<h1 style="text-align:center;"> ANALYSIS DATA KUNJUNGAN FASILITAS KESEHATAN DI INDONESIA 👩‍⚕️</h1>',
...
# Load data
# df = pd.read_csv("D:/winphyton/WPy64-31180/uasvisdatkel9/train.csv")
...
# Muat file GeoJSON
# geojson_path = "D:/winphyton/WPy64-31180/uasvisdatkel9/peta/indonesia-prov.geojson"
...
colTotalKunjungan, colTotalPasien = st.columns(2)
...
tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs(["Trend Kunjungan", "Status Pasien", "Status Rujukan", "Jenis Faskes dan Segmen", "Jenis Poli", "Provinsi Faskes", "Kepemilikan Akses", "Peta pasien"])
...
with tab1:
    st.markdown(
        "<h3>Banyaknya Berdasarkan Bulan</h3>",
        unsafe_allow_html=True,
    )

    # Check if the period start and end are the same
    if periodStart == periodEnd:
        # Filter data for the selected month and year
        filtered_df_single_month = filtered_df[
            (df["Tanggal Datang"].dt.year == int(yearStart)) &
            (df["Tanggal Datang"].dt.month == monthStartIndex)
        ]

        if not filtered_df_single_month.empty:
            trendByMonth = (
                filtered_df_single_month.groupby(pd.Grouper(key="Tanggal Datang", freq="D"))
                .count()
                .reset_index()
            )
            ...
            chartLineTrend = px.line(
                ...
                y="index",
                labels={
                    "Tanggal Datang": "Trend Kunjungan Berdasarkan Hari",
        ...
        if not trendByMonth.empty:
            chartLineTrend = px.line(
                ...
                y="index",
                labels={
                    "Tanggal Datang": "Trend Kunjungan Berdasarkan Bulan",
                    "index": "Jumlah Pengunjung",
                },
                markers=True,
            )
